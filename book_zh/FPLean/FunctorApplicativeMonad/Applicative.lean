...
使用 {anchorName checkInput}`checkInput` 进行表单验证，展示了 {anchorName ApplicativeNames}`Applicative` 相对于 {anchorName MonadExtends}`Monad` 的一个关键优点。
由于 {lit}`>>=` 具有足够的能力，可以根据第一步得到的值修改程序其余部分的执行方式，所以它_必须_从第一步接收一个值并继续传递。
如果没有收到值（例如因为发生了错误），那么 {lit}`>>=` 就无法执行程序的其余部分。
{anchorName Validate}`Validate` 展示了为什么即使如此也运行程序的其余部分可能有用：在不需要先前数据的情况下，运行程序的其余部分可以产生有用信息（在这里，就是更多验证错误）。
{anchorName ApplicativeNames}`Applicative` 的 {lit}`<*>` 可以在重新组合结果之前运行它的两个参数。
与之相对，{lit}`>>=` 会强制顺序执行。
每一步都必须完成，下一步才可以运行。
这通常很有用，但它使程序无法根据实际数据依赖自然地产生不同线程的并行执行。
像 {anchorName MonadExtends}`Monad` 这样更强大的抽象增加了 API 使用者可用的灵活性，但减少了 API 实现者可用的灵活性。
